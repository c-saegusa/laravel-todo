                ...
                <div class="col col-md-offset-3 col-md-6">
                    <nav class="panel panel-default">
                        ...
                        <div class="panel-body">
                            @if($errors->has('content'))<div class="alert alert-danger">{{ $errors->first('content') }}</div>@endif
                            @if($errors->has('status'))<div class="alert alert-danger">{{ $errors->first('status') }}</div>@endif
                            @if($errors->has('due_date'))<div class="alert alert-danger">{{ $errors->first('due_date') }}</div>@endif
                            <form action="{{ route('memos.update', $memo->id) }}" method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="content">メモ</label>
                                    <input type="text" class="form-control" name="content" id="content" value="{{ $memo->content }}">
                                </div>
                                <div class="form-group">
